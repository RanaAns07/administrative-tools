            ...
            <div className="flex p-1 bg-gray-100 rounded-lg w-full max-w-sm">
                <button
                    ...
                    className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${
                        activeTab === 'STANDARD' ? 'bg-white text-leads-blue shadow-sm' : 'text-gray-500 hover:text-gray-700'
                    }`}
                >
                    Full Fee Submissions
                </button>
                <button
                    onClick={() => setActiveTab('INSTALLMENTS')}
                    className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${
                        activeTab === 'INSTALLMENTS' ? 'bg-white text-leads-blue shadow-sm' : 'text-gray-500 hover:text-gray-700'
                    }`}
                >
                    Active Installments
                </button>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 border-b border-gray-100 text-xs uppercase text-gray-500 font-semibold tracking-wider">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-right">{activeTab === 'STANDARD' ? 'Total' : 'Installment'}</th>
                                <th className="px-4 py-3 text-right">Paid</th>
                                <th className="px-4 py-3 text-right">{activeTab === 'STANDARD' ? 'Outstanding' : 'Remaining'}</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            ...
                            ) : invoices.filter(inv => activeTab === 'STANDARD' ? inv.status !== 'PARTIAL' : inv.status === 'PARTIAL').length === 0 ? (
                                <tr><td colSpan={10} className="py-10 text-center text-gray-400 text-sm">No fee invoices found for this view.</td></tr>
                            ) : invoices.filter(inv => activeTab === 'STANDARD' ? inv.status !== 'PARTIAL' : inv.status === 'PARTIAL').map((inv, i) => (
